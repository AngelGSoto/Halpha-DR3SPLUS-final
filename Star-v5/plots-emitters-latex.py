...
for i in tab:
    for j in file_list1:
        if  i["ID"].endswith(" '"):
            if i["ID"].split("R3.")[-1].split(" ")[0].replace(".", "-") == j.split("s/")[-1].split("_")[0]:
                idd = i["ID"].split("b'")[-1].split(" ")[0]
                spec_ = "S-spectra/photopectrum_splus_" + i["ID"].split("R3.")[-1].split(" ")[0].replace(".", "-") + "_Halpha-DR3-SPLUS-PStotal-STAR-13r16-v5_PStotal.pdf"
                id_.append(idd)
...
latexname = "table-plots-{}-{}.tex".format(file_.split("-DR3")[0], file_.split("STAR-")[-1].split("-v5")[0])
