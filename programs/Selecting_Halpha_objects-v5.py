...
merged_table_list = []
for chunk in pd.read_csv(file_, chunksize=1000000):
    merged_table_list.append(chunk)
    
df = pd.concat(merged_table_list)
...
with sns.axes_style('white'):
    ...
    file_save = "diagram-{}-v2.jpg".format(file_.split('.cs')[0])
...
print(len(data_orig_fin))
asciifile = "Halpha-{}-v2.ecsv".format(file_.split('.cs')[0]) 
