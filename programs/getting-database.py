...
for k, t in zip(riii, rfff):
    Query = f"""SELECT Field, ID, RA, DEC, FWHM, ISOarea, KRON_RADIUS, MU_MAX, nDet_PStotal, PhotoFlagDet, 
        	  ...
        	  FROM "idr3"."all_idr3"
                  WHERE CLASS_STAR >= 0.5 AND PhotoFlag_r = 0 AND PhotoFlag_J0660 = 0 AND PhotoFlag_i = 0 AND r_PStotal >= """ + str(k) + """ AND r_PStotal < """  + str(t)  +  """ AND e_r_PStotal <= 0.2 AND e_J0660_PStotal <= 0.2 
                  AND e_i_PStotal <= 0.2"""
    # Executing the query
...
df.to_csv("DR3-SPLUS-PStotal-STAR-{}r{}-v5.csv".format(int(ri), int(rf)), index = False)
